namespace Testing.Automation.API.Builders.HttpActionResults.Content
{
    using System;
            ...
            RuntimeBinderValidator.ValidateBinding(() =>
            {
                ...
                if (actualStatusCode != statusCode)
                {
                    throw new VerificationException(string.Format(
                        "<pre> When calling {0} action in {1} expected to have {2} ({3}) status code, but received {4} ({5}).</pre>",
                        this.ActionName,
